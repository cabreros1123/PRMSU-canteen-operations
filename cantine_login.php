...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "SELECT * FROM cantines WHERE BINARY username = ?";
    ...
    if ($result->num_rows > 0) {
        ...
        if ($row['password'] === $password) {
            ...
            if ($row['active'] == 0 && $row['del_status'] == 0) {
                // Store necessary data in the session
                $_SESSION['username'] = $row['username']; // Store username
                $_SESSION['cantine_name'] = $row['name']; // Store cantine name
                $_SESSION['id_cantine'] = $row['id']; // Store id_cantine

                // Update the last_login column for the current cantine
                $updateSql = "UPDATE cantines SET last_login = NOW() WHERE BINARY username = ?";
                $updateStmt = $conn->prepare($updateSql);
                $updateStmt->bind_param('s', $username);
                $updateStmt->execute();
                $updateStmt->close();

                header("Location: /POS-PHP/user_cantine/home.php");
                exit();
            } elseif ($row['active'] == 1) {
...
<div class="login-box">
    ...
    <div class="header-title">Canteen Login</div>
